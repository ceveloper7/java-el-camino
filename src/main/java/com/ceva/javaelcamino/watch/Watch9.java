 ...
 * @author PC
 */
public class Watch9 extends JPanel {
    ...
    private int minute;
    private int second;
    private int ampm; // 0 == dia y 1 = noche
...
//        g2d.setFont(g2d.getFont().deriveFont(36f));
        ...
        // 1er arco
        Arc2D arc = new Arc2D.Float(centerX-dayNightRadius, centerY-dayNightRadius, dayNightRadius*2, dayNightRadius*2, 10, 180, Arc2D.CHORD);
        Area dnClip = new Area(arc);
        
        // 2do arco
        arc = new Arc2D.Float(centerX-dayNightRadius*1.1f, centerY-dayNightRadius*1.1f, dayNightRadius*2.2f, dayNightRadius*2.2f, 170, 180, Arc2D.CHORD);
        dnClip.subtract(new Area(arc));
        
        // 3er arco
        arc = new Arc2D.Float(centerX-dayNightRadius*0.2f, centerY-dayNightRadius*0.2f, dayNightRadius*0.4f, dayNightRadius*0.4f, 0, 180, Arc2D.CHORD);
        dnClip.subtract(new Area(arc));
        /**** Redonde de las esquinas ***********/
        final float smoothLen = 0.15f;
        // dibujamos una region
        int x1 = (int)(centerX + dayNightRadius * (1 - smoothLen) * Math.cos(Math.toRadians(10)));
        int y1 = (int)(centerY - dayNightRadius * (1 - smoothLen) * Math.sin(Math.toRadians(10)));
        
        int x2 = (int)(centerX + dayNightRadius * Math.cos(Math.toRadians(10)));
        int y2 = (int)(centerY - dayNightRadius * Math.sin(Math.toRadians(10)));
        
        int x3 = (int)(centerX + dayNightRadius * Math.cos(Math.toRadians(10) + smoothLen));
        int y3 = (int)(centerY - dayNightRadius * Math.sin(Math.toRadians(10) + smoothLen));
        
        int x4 = (int)(centerX + dayNightRadius * 1.05f * Math.cos(Math.toRadians(10) + smoothLen));
        int y4 = (int)(centerY - dayNightRadius * 1.05f * Math.sin(Math.toRadians(10) + smoothLen));
        
        // Agregamos la nueva cuerva para redondear el angulo derecho
        ...
        gp.lineTo(x4, y4);
        x2 = (int)(centerX + dayNightRadius * 1.05f * Math.cos(Math.toRadians(10-1)));
        y2 = (int)(centerY - dayNightRadius * 1.05f * Math.sin(Math.toRadians(10-1)));
        gp.lineTo(x2, y2);
        // cerramos la curva
        gp.closePath();
        dnClip.subtract(new Area(gp));
        
        // Agregamos la nueva cuerva para redondear el angulo izquierdo
        // dibujamos una region
        x1 = (int)(centerX - dayNightRadius * (1 - smoothLen) * Math.cos(Math.toRadians(10)));
        y1 = (int)(centerY - dayNightRadius * (1 - smoothLen) * Math.sin(Math.toRadians(10)));
        
        x2 = (int)(centerX - dayNightRadius * Math.cos(Math.toRadians(10)));
        y2 = (int)(centerY - dayNightRadius * Math.sin(Math.toRadians(10)));
        
        x3 = (int)(centerX - dayNightRadius * Math.cos(Math.toRadians(10) + smoothLen));
        y3 = (int)(centerY - dayNightRadius * Math.sin(Math.toRadians(10) + smoothLen));
        
        x4 = (int)(centerX - dayNightRadius * 1.05f * Math.cos(Math.toRadians(10) + smoothLen));
        y4 = (int)(centerY - dayNightRadius * 1.05f * Math.sin(Math.toRadians(10) + smoothLen));
        ...
        gp.closePath();
        dnClip.subtract(new Area(gp));
        /**** Fin de redondeo de las esquinas ***/
        
        Shape saveClip = g2d.getClip();
        g2d.setClip(dnClip);
        
        // giramos la caratula segun si es de dia o de noche
        // a medianoche hour24 es 0 / a medio dia hour24 es 12
        int hour24 = hour + 12 * ampm;
        double angle = (double)(180.0 + (360.0/24) * hour24 + (double)((360.0/24/60) * minute)) % 360;
        createDayNightImage((int)(dayNightRadius * 2));
        
        // guardamos el estado actual de las coordenadas
        AffineTransform saveTrans = g2d.getTransform();
        AffineTransform trans = new AffineTransform();
        // trasladamos el eje de coordenadas al centro del reloj
        trans.translate(centerX, centerY);
        trans.rotate(Math.toRadians(angle));
        // utilzams la nueva transformacion
        g2d.setTransform(trans);
        // todo lo que dibujemos aparecera traladado y rotado
        g2d.drawImage(dayNightImage, -dayNightImage.getWidth()/2, -dayNightImage.getHeight()/2, null);
